...
public class SAQLv2_002 {
    
    public int[] solution(int[] heights) {
        int rtAry[] = new int[heights.length];

        for (int sendTowerIdx = heights.length - 1; sendTowerIdx > 0; --sendTowerIdx) { // ~ 1
            for (int leftTowerIdx = sendTowerIdx - 1; leftTowerIdx > -1; --leftTowerIdx) { // ~ 0
                final int sendTowersHeight = heights[sendTowerIdx];
                final int leftTowersHeight = heights[leftTowerIdx];

                if (sendTowersHeight < leftTowersHeight) {
                    rtAry[sendTowerIdx] = leftTowerIdx + 1;
                    break;
                }
            }
        }
    ...
    public static void main(String[] args) {
        // ================================================================
        final SAQLv2_002 solution = new SAQLv2_002();
        final int[] heights = new int[] { 10, 9, 1, 1, 1, 1, 1 };
        // ================================================================
        final long bgnTime = System.currentTimeMillis();
        final int[] results = solution.solution(heights);
