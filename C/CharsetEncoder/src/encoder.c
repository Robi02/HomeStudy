...
int Encode(char *pCharset, char *pOptions, char *pInStr, char *pOutStr, int szOutStrBuf)
{
	...
	{
		fprintf(stderr, "EncodeError: Check pCharset|Options|String. \
						(pCharset=%s, pOptions=%s, pInStr=%s)\n", pCharset, pOptions, pInStr);
		return -1;
	}
	
	if ( (charsetIdx = GetCharsetIdx(pCharset)) < 0 )
	{
		fprintf(stderr, "EncodeError: Check pCharset|G_CHARSET. (pCharset=%s)\n", pCharset);
		return -1;
	}
	
	if ( (optionsIdx = GetOptionsIdx(pOptions)) < 0 )
	{
		fprintf(stderr, "EncodeError: Check pOptions|G_OPTIONS. (pOptions=%s)\n", pOptions);
		return -1;
	}
	
	if ( (pEncoderFunc = SelectEncodingFunc(charsetIdx, optionsIdx)) == NULL )
	{
		fprintf(stderr, "EncodeError: Cannot found correct En/Decoder function. \
	...
	{
		fprintf(stderr, "EncodeError: pEncoderFunc() error. (returns:%d)\n", rtVal);
		return -1;
	}
