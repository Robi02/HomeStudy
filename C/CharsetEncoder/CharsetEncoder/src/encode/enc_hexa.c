...
{
	unsigned char *pInByte, left4Bit, right4Bit, rtByte;
	int i;
	...
	for ( i = 0 ; i < 2; ++i )
	{
		...
		if ( *pInByte > 47 && *pInByte < 58 )
		{
			*pInByte -= 48; /* Char '0' mapped to decimal 0  */
		}
		else if ( *pInByte > 64 && *pInByte < 71 )
		{
			*pInByte -= 55; /* Char 'A' mapped to decimal 10 */
		}
		else
		{	/* Undefined symbol error */
			fprintf(stderr, "%s() Error: Undefined symbol '%c' in input string!\n", __FUNCTION__, *pInByte);
			return -2;
		}
	}
